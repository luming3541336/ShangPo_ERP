Public Class Set_Sale_Model
    Protected Const SELECT_PROD_REVIEW_BY_SALEID = "SELECT s.SalePID as SaleDetailID, s.Count,s.PurchasePID as PurchaseDetailID, PurchaseProd.Specification, PurchaseProd.Width,PurchaseProd.Length, PurchaseProd.CBM, PurchaseProd.Remark, ProdSet.Name,SupplierData.Name As Supplier,SupplierData.Abbr,s.PIC,(Select PurchaseProd.Count - SUM(ISNULL(SaleProd.Count, 0)) + s.Count As RemainAmount
                            ...
                            GROUP BY PurchaseProd.Count ) As RemainAmount FROM ProdSet INNER JOIN SupplierData On ProdSet.SuID = SupplierData.SuID INNER JOIN PurchaseProd  On ProdSet.ProdID = PurchaseProd.ProdID INNER JOIN
                            SaleProd s On PurchaseProd.PurchasePID = s.PurchasePID  WHERE s.SaleID = @editID"
    Protected Const SELECT_FIT_REVIEW_BY_SALEID = "SELECT Fit.SaleFID as SaleDetailID,Fit.PurchaseFID as PurchaseDetailID ,Fit.[Count],Fit.PIC,PurchaseFit.Width, PurchaseFit.Length, PurchaseFit.CBM, PurchaseFIt.Specification,PurchaseFIt.Remark,FittingSet.FitID,FittingSet.Name,SupplierData.Name as Supplier, SupplierData.Abbr ,(SELECT (PurchaseFit.[Count] - SUM(ISNULL(SaleFit.[Count], 0))) + Fit.Count AS RemainAmount
                           From PurchaseFit LEFT Join SaleFit ON PurchaseFit.PurchaseFID = SaleFit.PurchaseFID, FittingSet,SupplierData, PurchaseData
                           Where PurchaseFit.FitID = FittingSet.FitID And FittingSet.SuID = SupplierData.SuID  And PurchaseData.PurchaseID = PurchaseFit.PurchaseID AND PurchaseFIt.PurchaseFID = Fit.PurchaseFID
                           Group By PurchaseFit.[Count]) as RemainAmount From FittingSet, SupplierData,PurchaseFIt ,SaleFit as Fit
                           Where FittingSet.SuID = SupplierData.SuID AND PurchaseFIt.FitID = FittingSet.FitID AND Fit.PurchaseFID = PurchaseFIt.PurchaseFID AND SaleID = @editID"
    Protected Const SELECT_PROD_STOCK_BY_CASEID = "SELECT PurchaseProd.PurchasePID as PurchaseDetailID, ProdSet.[Name], SupplierData.[Name] AS Supplier,SupplierData.Abbr,PurchaseProd.Specification,PurchaseProd.[Count],Width,[Length],CBM,Remark,([PurchaseProd].[Count] - SUM(ISNULL(SaleProd.[Count], 0))) AS RemainAmount
                                              From PurchaseProd LEFT Join SaleProd ON PurchaseProd.PurchasePID = SaleProd.PurchasePID, ProdSet,SupplierData,PurchaseData
                                              Where PurchaseProd.ProdID = ProdSet.ProdID And ProdSet.SuID = SupplierData.SuID And PurchaseData.CaseID = @caseID And PurchaseData.PurchaseID = PurchaseProd.PurchaseID
    ...
    Protected Const SELECT_FIT_STOCK_BY_CASEID = "SELECT PurchaseFit.PurchaseFID as PurchaseDetailID, FittingSet.[Name], SupplierData.[Name] AS Supplier,SupplierData.Abbr,PurchaseFit.Width,PurchaseFit.[Length],PurchaseFit.CBM,PurchaseFit.Specification,PurchaseFit.[Count],Remark,(PurchaseFit.[Count] - SUM(ISNULL(SaleFit.[Count], 0))) AS RemainAmount
                                              From PurchaseFit LEFT Join SaleFit ON PurchaseFit.PurchaseFID = SaleFit.PurchaseFID, FittingSet,SupplierData, PurchaseData
                                               Where PurchaseFit.FitID = FittingSet.FitID And FittingSet.SuID = SupplierData.SuID And PurchaseData.CaseID = @caseID And PurchaseData.PurchaseID = PurchaseFit.PurchaseID
